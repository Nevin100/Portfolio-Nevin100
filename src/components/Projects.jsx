...
function ProjectCard({ project, index, expandedIndex, toggleExpand, big }) {
  return (
    <motion.div
      layout
      key={project.title}
      className={`group relative flex flex-col bg-base-200/40 border border-base-content/10 rounded-3xl overflow-hidden hover:border-base-content/20 transition-all duration-300 ${big ? "md:col-span-1" : ""}`}
    >
      <div className={`relative ${big ? "h-56" : "h-52"} overflow-hidden cursor-pointer`}>
        {project.image ? (
          ...
          <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary/20 via-accent/10 to-secondary/20">
            <GraduationCap className="text-6xl text-primary/60" size={56} />
          </div>
        )}
        ...
        <a
          ...
          className="absolute inset-0 bg-neutral/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-[2px]"
        >
          <div className="bg-base-content text-base-100 px-4 py-2 rounded-full flex items-center gap-2 font-bold transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
            Live Preview <FiExternalLink />
          </div>
        </a>

        {project.featuredTag && (
          <div className="absolute top-3 left-3 z-20 text-[10px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full bg-primary/90 text-primary-content shadow-lg">
            {project.featuredTag}
          </div>
        )}
        ...
        <div className="absolute top-3 right-3 z-20">
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="p-2 bg-base-200/80 backdrop-blur-md rounded-full text-base-content hover:text-primary transition-colors block"
          >
            <FiGithub size={18} />
          </a>
        </div>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <button
          ...
          className="flex items-center gap-2 text-sm font-semibold text-primary hover:text-primary/80 transition-colors mt-auto"
        >
...
function Projects() {
  ...
  return (
    <>
      ...
      <section id="projects" className="relative py-24 px-6 lg:px-20 bg-base-100 overflow-hidden">
          ...
          <div className="text-center mb-16">
            ...
            <div className="relative max-w-md mx-auto group">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-base-200/50 border border-base-content/10 rounded-2xl text-base-content backdrop-blur-xl focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all shadow-2xl"
              />
            </div>
          </div>
            ...
            <>
              ...
              {filteredProjects.length === 0 && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-20">
                  <p className="text-base-content/50 text-xl font-medium">No projects match your search.</p>
                </motion.div>
              )}
            </>
          ) : (
