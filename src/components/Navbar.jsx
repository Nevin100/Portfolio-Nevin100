  ...
  return (
    <header className="fixed top-0 inset-x-0 z-[100] px-4 md:px-8 py-5 transition-all duration-300">
      ...
      >
          ...
          <div className="leading-none">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="text-[10px] uppercase tracking-[0.35em] text-base-content/40 font-bold ml-1 mt-1"
              >
                {terminalWords[index]}
              </motion.div>
            </AnimatePresence>
        ...
        {/* Mobile — ThemeSwitcher + Hamburger side by side */}
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -20 }}
            className="lg:hidden absolute inset-x-4 mt-3 bg-base-100/95 backdrop-blur-2xl border border-base-content/10 rounded-2xl shadow-2xl overflow-hidden"
          >
            {/* Nav Links */}
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={`px-4 py-3 rounded-xl text-sm font-bold uppercase tracking-widest text-center transition-colors
                      ${isActive
                        ? "bg-primary/10 text-primary border border-primary/20"
                        : "bg-base-200/50 text-base-content/70 border border-base-content/5 hover:text-base-content"
                      }`}
                  >
                    {item}
                  </Link>
              ...
              <div className="flex gap-5 text-2xl">
                {socialIcons.map(({ href, label, icon, color }) => (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`text-base-content/60 transition-colors ${color}`}
                  >
``                    {icon}
                  </a>
                ))}
              ...
              <button
                onClick={() => setIsOpen(false)}
                className="text-xs font-bold uppercase tracking-widest text-base-content/40 hover:text-base-content transition-colors"
              >
                Close ✕
              </button>
            </div>
